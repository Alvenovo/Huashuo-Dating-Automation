...
def step_node_env_template() -> Step:
    ...
    **不生成任何真实凭据**，只在文件不存在时放一份带注释的模板并提示要填什么。
    ...
    template = "\n".join([
        ...
        "# 格式：KEY=VALUE（值不用加引号）。",
        "",
        "# ⚠️ **换测试号只改本文件的 HALL_TEST_USER**（密码不同就一并改 HALL_TEST_PASSWORD）。",
        "#    控制机侧一处都不用动：凭据按设计只放节点本地，不进共享盘、不进任务文件。",
        "#    改完**不必重跑 bootstrap、也不必重起 agent** —— 每个套件开跑前会重读本文件。",
        "#    ⚠️ 但**窗口里的 $env:HALL_TEST_USER 优先级更高、会盖掉本文件** ——",
        "#       改完请到**新开的窗口**里起 agent（旧窗口里设过的话会一直是旧号）。",
        "",
        "# --- 登录套件（P1-B）---",
        "# 密码登录用例",
        "HALL_TEST_USER=",
